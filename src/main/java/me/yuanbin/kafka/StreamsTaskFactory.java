...
public class StreamsTaskFactory {
    ...
    private static final Logger logger = LoggerFactory.getLogger(StreamsTaskFactory.class);
    ...
    private static Properties getKafkaProps(String appId) {
        String defaultAppId = appConfig.getString(DEFAULT_APPLICATION_ID);
        String kafkaAppId = Strings.isNullOrEmpty(appId) ? defaultAppId : appId;
        String bootstrapServers = appConfig.getString(DEFAULT_BOOTSTRAP_SERVERS);
        return getKafkaProps(kafkaAppId, bootstrapServers);
    }
